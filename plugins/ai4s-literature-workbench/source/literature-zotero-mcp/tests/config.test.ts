...
describe('loadConfig', () => {
  it('applies defaults with an empty env', () => {
    ...
    expect(cfg.dataDir).toContain('literature-zotero-mcp');
  });

  it('prefers the derivative data-directory override while accepting the legacy alias', () => {
    expect(
      loadConfig({ LITERATURE_ZOTERO_MCP_DATA_DIR: '/tmp/literature' } as NodeJS.ProcessEnv)
        .dataDir,
    ).toBe('/tmp/literature');
    expect(loadConfig({ ZOTEUS_DATA_DIR: '/tmp/legacy' } as NodeJS.ProcessEnv).dataDir).toBe(
      '/tmp/legacy',
    );
  });
  ...
  it('supports the minimal literature workbench tool profile', () => {
    const cfg = loadConfig({
      LITERATURE_ZOTERO_MCP_TOOL_PROFILE: 'workbench',
    } as NodeJS.ProcessEnv);
    expect(cfg.toolProfile).toBe('workbench');
  });
