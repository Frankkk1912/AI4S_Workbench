    ...
    actions: [
      {
        ...
        match: null,
        priority_recommendation: { level: 1, reason_codes: ['selected-default'], tag_decision: 'add' },
      },
      {
        ...
        priority_recommendation: { level: 3, reason_codes: ['core-conclusion'], tag_decision: 'add' },
      },
    ],
...
function context(overrides: Record<string, any> = {}) {
  const web = {
    ...
    getItem: vi.fn(async () => ({
      ...
      version: 3,
      data: {
        itemType: 'journalArticle', title: 'Reuse paper', DOI: '10.1/reuse', date: '2024',
        collections: ['OLD'], tags: [{ tag: 'existing' }],
      },
    })),
    writeCollections: vi.fn(async () => ({ successful: [{ index: 0, key: 'COLL001', version: 1 }], failed: [], unchanged: [], newLibraryVersion: 4 })),
    writeItems: vi.fn(async () => ({ successful: [{ index: 0, key: 'NEW001', version: 4 }], failed: [], unchanged: [], newLibraryVersion: 4 })),
...
describe('zotero import-plan execution', () => {
  ...
  it('creates and reuses items, persists a complete receipt, and no-ops on repeat', async () => {
    ...
    expect(receipt.results.map((result: any) => result.outcome).sort()).toEqual(['created', 'reused']);
  ...
  it('preserves a live user Priority added after plan generation', async () => {
    ...
    const ctx = context({
      getItem: vi.fn(async () => ({
        key: 'EXIST001', version: 3,
        data: {
          itemType: 'journalArticle', title: 'Reuse paper', DOI: '10.1/reuse', date: '2024',
          collections: ['OLD'], tags: [{ tag: 'existing' }, { tag: 'AI4S:Priority:2' }],
        },
      })),
    });
  ...
  it('persists retryable batch failure as blocked pending work', async () => {
    ...
    error.status = 429;
    const ctx = context({ writeItems: vi.fn(async () => { throw error; }) });
    const result = await applyImportPlan(ctx, plan(), lib, join(directory, 'receipt.json'));
  ...
  it('turns a new live match for planned create into check', async () => {
    ...
    const ctx = context({
      listItems: vi.fn(async () => ({
        data: [{ key: 'LIVE001', version: 1, data: { title: 'Create paper', DOI: '10.1/create', date: '2025' } }],
        totalResults: 1,
        lastModifiedVersion: 4,
      })),
    });
