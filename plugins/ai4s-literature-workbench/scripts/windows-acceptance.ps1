...
Invoke-Check -Name "codex-plugin-visible" -Action {
    ...
    if (($pluginList -join "`n") -notmatch "ai4s-literature-workbench") {
        throw "ai4s-literature-workbench is not visible in the configured Codex marketplaces."
    }
}
...
Invoke-Check -Name "bundle-drift-check" -Action {
    Invoke-InDirectory -Directory $RepoRoot -Action {
        Invoke-External -Command "node" -Arguments @("plugins/ai4s-literature-workbench/scripts/assemble-plugin.mjs", "check")
    }
}
