...
const workbenchSourceRoot = "plugins/ai4s-literature-workbench/source";
const bundles = [
	...
	[`${workbenchSourceRoot}/literature-zotero-mcp`, "runtime"],
	[`${workbenchSourceRoot}/literature-fulltext-mcp`, "fulltext-runtime"],
];
...
const fulltextRuntimeSource = resolve(
	repoRoot,
	workbenchSourceRoot,
...
function bundleRuntime() {
	...
	if (!esbuild) {
		throw new Error(
			"Missing literature-zotero-mcp esbuild dependency; run npm install in plugins/ai4s-literature-workbench/source/literature-zotero-mcp first.",
		);
	}
	...
	if (isNativeBinary) {
		...
		execFileSync(process.execPath, [esbuild, ...esbuildArgs], {
			stdio: "inherit",
		});
	}
}
...
if (command === "sync") {
	for (const [source, destination] of bundles) {
		...
		if (source === `${workbenchSourceRoot}/literature-zotero-mcp`)
			bundleRuntime();
	}
	const manifest = {
	...
	process.stdout.write(
		`Synced ${bundles.length} plugin bundles from ${repoRoot}\n`,
	);
} else {
	const drift = [];
	for (const [source, destination] of bundles) {
