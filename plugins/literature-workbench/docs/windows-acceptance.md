...
```powershell
cd "C:\path with spaces\AI4S_Workbench\plugins\literature-workbench"
...
1. Run `node .\runtime\scripts\zotero-configure.mjs setup` locally. Confirm the
   key is stored under `%LOCALAPPDATA%\literature-workbench` and is absent
...
### Recorded Fulltext live result — 2026-08-07

A known-OA item in the dedicated acceptance collection completed the reviewed
OA-only handoff workflow. The compact redacted result is:

| Fulltext tool status | Verified handoffs | Child PDF attachments | Actionable failure code |
| --- | ---: | ---: | --- |
| complete | 1 | 1 | `DOWNLOAD_NOT_PDF` on the PMC route; reviewed public-PDF fallback verified |

No credential, identifier, item key, PDF path, URL, or Zotero content is
recorded here.
