...
export function defaultPrivateEnvFile(
  ...
  if (platform === 'win32') {
    ...
    return platformPath.join(localAppData, 'literature-workbench', 'zotero.env');
  }

  if (platform === 'darwin') {
    return platformPath.join(home, 'Library', 'Application Support', 'literature-workbench', 'zotero.env');
  }

  const configHome = env.XDG_CONFIG_HOME?.trim() || platformPath.join(home, '.config');
  return platformPath.join(configHome, 'literature-workbench', 'zotero.env');
}

/** Previous plugin-identity location retained for credential migration. */
