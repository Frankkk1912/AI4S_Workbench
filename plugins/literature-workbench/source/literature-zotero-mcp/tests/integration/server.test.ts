...
async function connect() {
  const server = new McpServer(
    { name: 'literature-zotero-mcp-test', version: '0.0.0' },
...
describe('Literature Zotero MCP server (in-process)', () => {
  it('lists all forty-one tools', async () => {
  ...
  it('lists the workflow prompts', async () => {
    ...
    expect(names).toContain('zotero-semantic-tag');
    expect(names).toContain('zotero-ai-summary');
