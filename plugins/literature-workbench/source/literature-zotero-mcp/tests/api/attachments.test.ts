...
import { uploadFile, downloadFile, guessContentType, resumeUploadFile } from '../../src/api/attachments.js';
...
describe('attachments', () => {
  ...
  it('resumes bytes/register for an existing attachment key without creating another item', async () => {
    const fp = join(tmpdir(), `zoteus-resume-${process.pid}.txt`);
    await writeFile(fp, 'resume');
    const web = fakeWeb();
    const phases: string[] = [];
    await resumeUploadFile(web, lib, { filePath: fp, attachmentKey: 'ATT1' }, async (phase) => { phases.push(phase); });
    expect(web.writeItems).not.toHaveBeenCalled();
    expect(web.registerUpload).toHaveBeenCalledWith(lib, 'ATT1', 'UP1');
    expect(phases).toEqual(['bytes_uploaded', 'registered']);
    await rm(fp);
  });
