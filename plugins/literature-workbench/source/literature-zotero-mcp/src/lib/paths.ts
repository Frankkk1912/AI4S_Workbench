...
export function defaultFulltextExchangeRoot(
  ...
  if (platform === 'win32') {
    ...
    return platformPath.join(localAppData, 'literature-workbench', 'fulltext-exchange');
  }
  if (platform === 'darwin') {
    return platformPath.join(home, 'Library', 'Application Support', 'literature-workbench', 'fulltext-exchange');
  }
