...
const repoRoot = resolve(
	process.env.AI4S_REPO_ROOT || resolve(pluginRoot, "../.."),
);
...
const skillSourceRoot = resolve(
	process.env.AI4S_SKILLS_ROOT ||
		(existsSync(resolve(pluginRoot, "source-skills"))
			? resolve(pluginRoot, "source-skills")
			: repoRoot),
);
const bundles = [
...
].map((source) => [source, `skills/${source}`]);
...
const meta = readJson(metaPath, "plugin-meta.json");
for (const key of [
	"name",
	"description",
	"author",
	...
	if (meta[key] === undefined)
		throw new Error(`plugin-meta.json missing required key: ${key}`);
}
if (!/^\d+\.\d+\.\d+$/.test(packageManifest.version)) {
	throw new Error(
		`package.json version must be plain semver (got ${packageManifest.version}).`,
	);
}
if (meta.version !== packageManifest.version) {
	throw new Error("plugin-meta.json version must match package.json version.");
}
if (meta.name !== basename(pluginRoot)) {
	throw new Error(
		`plugin-meta.json name must match the bundle directory: ${meta.name} != ${basename(pluginRoot)}`,
	);
}
...
const generated = [
	[
		...
		() => ({
			...baseManifest(),
			skills: "./skills/",
			interface: meta.codex.interface,
		}),
	],
...
function bundleEntries() {
	return bundles.map(([source, destination]) => ({
		source,
		destination,
		sha256: treeHash(resolve(skillSourceRoot, source)),
	}));
}
	...
	if (existsSync(manifestPath)) {
		try {
			...
			if (
				...
				current.source_root === sourceRoot &&
				JSON.stringify(current.bundles) === JSON.stringify(entries) &&
				typeof current.generated_at === "string"
			)
				generatedAt = current.generated_at;
		} catch {
			/* Invalid generated output is replaced during sync. */
		}
	}
	return {
		schema_version: "1.0",
...
export function syncPlugin() {
	for (const [source, destination] of bundles) {
		const from = resolve(skillSourceRoot, source);
		const to = resolve(pluginRoot, destination);
		if (!existsSync(from)) throw new Error(`Missing source skill: ${from}`);
		rmSync(to, { recursive: true, force: true });
		cpSync(from, to, { recursive: true, filter: (path) => !excluded(path) });
	}
	...
	writeFileSync(
		resolve(pluginRoot, "bundle-manifest.json"),
...
export function checkPluginAssembly() {
	const drift = bundles
		.filter(
			([source, destination]) =>
				treeHash(resolve(skillSourceRoot, source)) !==
				treeHash(resolve(pluginRoot, destination)),
		)
		.map(([source, destination]) => `${source} -> ${destination}`);
	for (const [path, render] of generated)
		if (!semanticJsonEquals(path, renderManifest(render)))
			drift.push(`manifest:${relative(pluginRoot, path)}`);
	if (drift.length)
		throw new Error(
			`Bundled plugin content differs from source-of-truth:\n${drift.join("\n")}`,
		);
...
if (isMain) {
	...
	process.stdout.write(
		`${command === "sync" ? syncPlugin() : checkPluginAssembly()}\n`,
	);
}
