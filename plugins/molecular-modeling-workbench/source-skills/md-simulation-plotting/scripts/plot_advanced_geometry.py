...
def plot_tier5_geometry(
    sys1_dist_xvg,
    sys2_dist_xvg=None,
    output_name="tier5_geometry",
    sys1_label="System 1",
    sys2_label="System 2",
    ...
    fig, axes = plt.subplots(
        1,
        2,
        figsize=md_style.StyleConfig.FIG_SIZE_GEOMETRY,
    ...
    ax1.plot(
        t1[49:], sm1, color=md_style.StyleConfig.SYS1_COLOR, lw=1.2, label=sys1_label
    )
    ...
    ax2.plot(
        x_range, y1, color=md_style.StyleConfig.SYS1_COLOR, lw=1.5, label=sys1_label
    )
    ...
    if not is_single:
        ...
        ax2.plot(
            x_range, y2, color=md_style.StyleConfig.SYS2_COLOR, lw=1.5, label=sys2_label
        )
