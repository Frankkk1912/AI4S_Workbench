...
def main():
        ...
        parser.add_argument(
                "--style-config",
                type=str,
                help="Path to JSON configuration file to override default colors/fonts.",
        )
        ...
        def add_common_args(p):
                p.add_argument(
                        "--sys1-label", default="System 1", help="Label for System 1"
                )
                p.add_argument(
                        "--sys2-label", default="System 2", help="Label for System 2"
                )
        ...
        p_basic = subparsers.add_parser(
                "basic",
                help="Plot all standard reporting figures (QC, Hbonds, DSSP) automatically",
        )
        p_basic.add_argument(
                "--sys1-dir",
                required=True,
                help="Directory containing basic analysis outputs for System 1",
        )
        p_basic.add_argument(
                "--sys2-dir",
                default=None,
                help="Directory containing basic analysis outputs for System 2",
        )
        p_basic.add_argument(
                "--eq-start-ns",
                type=float,
                default=20.0,
                help="Equilibration start time in ns",
        )
        p_basic.add_argument(
                "--rmsd-ylabel",
                default="Backbone RMSD (nm)",
                help="Y-axis label for RMSD",
        )
        p_basic.add_argument(
                "--rmsf-ylabel", default="Cα RMSF (nm)", help="Y-axis label for RMSF"
        )
        p_basic.add_argument(
                "--rmsf-xlabel", default="Residue Number", help="X-axis label for RMSF"
        )
        add_common_args(p_basic)

        # --- Formal diagnostics gallery (M5) ---
        p_diagnostics = subparsers.add_parser(
                "diagnostics",
                help="Plot temperature/pressure/energy and RMSD/RMSF diagnostics",
        )
        p_diagnostics.add_argument(
                "--analysis-dir",
                required=True,
                help="Directory containing diagnostics XVG files",
        )
        p_diagnostics.add_argument(
                "--eq-start-ns",
                type=float,
                default=0.0,
                help="Equilibration marker (ns)",
        )
        p_diagnostics.add_argument("--output", required=True, help="Output file prefix")

        # --- Advanced: FEL ---
        p_fel = subparsers.add_parser(
                "advanced-fel", help="Plot 2D Free Energy Landscape (PCA projections)"
        )
        ...
        p_fel.add_argument(
                "--temp", type=float, default=300.0, help="Simulation temperature (K)"
        )
        add_common_args(p_fel)

        # --- Advanced: Network ---
        p_net = subparsers.add_parser(
                "advanced-network", help="Plot Contact Maps and DCCM matrices"
        )
        ...
        p_geom = subparsers.add_parser(
                "advanced-geometry",
                help="Plot targeted Distance/Angle density distributions",
        )
        p_geom.add_argument(
                "--sys1-dist", required=True, help="System 1 distance/angle XVG"
        )
        p_geom.add_argument(
                "--sys2-dist", default=None, help="System 2 distance/angle XVG"
        )
        p_geom.add_argument(
                "--metric-name", default="Distance", help="Name of the metric"
        )
        p_geom.add_argument("--unit", default="nm", help="Unit of the metric")
        add_common_args(p_geom)

        # --- Advanced: RDF ---
        p_rdf = subparsers.add_parser(
                "advanced-rdf", help="Plot Radial Distribution Function g(r)"
        )
        ...
        p_rdf.add_argument(
                "--target-name", default="Target", help="Name of target molecule"
        )
        ...
        if args.style_config:
                try:
                        ...
                        print(
                                f"Warning: Failed to load style config. {e}",
                                file=sys.stderr,
                        )

        try:
                if args.command == "basic":
                        print("--- Running Basic Plotting Pipeline ---")
                        ...
                        s1_dssp = resolve_file(
                                args.sys1_dir, "dssp.xpm"
                        ) or resolve_file(args.sys1_dir, "dssp.dat")
                        ...
                        s2_dssp = resolve_file(
                                args.sys2_dir, "dssp.xpm"
                        ) or resolve_file(args.sys2_dir, "dssp.dat")
                        ...
                        if all([s1_rmsd, s1_rmsf, s1_rg, s1_sasa]):
                                ...
                                print(
                                        ">> Missing some QC files (rmsd, rmsf, rg, sasa). Skipping 4-Panel QC."
                                )
                        ...
                        plot_diagnostics(
                                args.analysis_dir, args.output, args.eq_start_ns
                        )
