...
class EnvironmentCliTests(unittest.TestCase):
    def test_audit_and_plan_write_structured_artifacts(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                report["artifact_type"], "molecular_modeling_environment_report"
            )
    ...
    def test_cpu_gromacs_plan_uses_a_managed_prefix(self):
        ...
        self.assertEqual(
            plan["actions"][0]["prefix"], str(root / "environments" / "gromacs")
        )
        self.assertEqual(
            plan["actions"][0]["smoke_test"],
            [str(root / "environments" / "gromacs" / "bin" / "gmx"), "--version"],
        )
    ...
    def test_gpu_plan_installs_the_pinned_mmpbsa_analysis_environment(self):
        ...
        self.assertEqual(
            action["smoke_test"],
            [
                str(root / "environments" / "gmx-mmpbsa" / "bin" / "gmx_MMPBSA"),
                "--version",
            ],
        )
    ...
    def test_dssp_plan_uses_dssp_package_and_mkdssp_binary(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            action = ENVIRONMENT.build_plan("cpu-fallback", ["dssp"], root)["actions"][
                0
            ]
        ...
        self.assertEqual(
            action["smoke_test"],
            [str(root / "environments" / "dssp" / "bin" / "mkdssp"), "--version"],
        )
    ...
    def test_bootstrap_rejects_a_signed_legacy_dssp_plan(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.assertRaisesRegex(
                ENVIRONMENT.EnvironmentError, "Legacy DSSP plan"
            ):
    ...
    def test_acpype_plan_uses_a_dedicated_ambertools_prefix(self):
        ...
        self.assertEqual(
            action["smoke_test"],
            [str(root / "environments" / "acpype" / "bin" / "acpype"), "--version"],
        )

    def test_plan_normalizes_managed_prefixes_to_absolute_paths(self):
        plan = ENVIRONMENT.build_plan(
            "cpu-fallback", ["vina"], Path("relative-environment")
        )
    ...
    def test_bootstrap_action_failure_writes_receipt_and_cli_exits_nonzero(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            ENVIRONMENT.write_json(
                plan_path, ENVIRONMENT.build_plan("cpu-fallback", ["vina"], root)
            )
            failed = subprocess.CompletedProcess(
                args=[], returncode=7, stdout="solver output", stderr="package missing"
            )
            stdout = io.StringIO()
            stderr = io.StringIO()
            argv = [
            ...
            with (
                ...
                mock.patch.object(
                    ...
                    "which",
                    side_effect=lambda name: (
                        "/usr/bin/micromamba" if name == "micromamba" else None
                    ),
                ),
                mock.patch.object(ENVIRONMENT.subprocess, "run", return_value=failed),
                mock.patch.object(ENVIRONMENT.time, "sleep"),
    ...
    def test_bootstrap_smoke_failure_writes_receipt_and_cli_exits_nonzero(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            smoke_failed = subprocess.CompletedProcess(
                args=[], returncode=9, stdout="", stderr="cannot start"
            )
            argv = [
                str(SCRIPT),
                "bootstrap",
            ...
            with (
                ...
                mock.patch.object(
                    ENVIRONMENT.subprocess, "run", side_effect=[installed, smoke_failed]
                ),
    ...
    def test_audit_discovers_acpype_in_its_managed_prefix(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                mock.patch.object(ENVIRONMENT.shutil, "which", return_value=None),
                mock.patch.object(
                    ENVIRONMENT, "command_version", side_effect=fake_version
                ),
                mock.patch.object(
                    ENVIRONMENT,
                    "run_probe",
    ...
    def test_audit_discovers_mmpbsa_gromacs_and_reports_trajectory_capability(self):
        ...
        self.assertEqual(
            report["trajectory_analysis"]["gromacs"]["path"], str(gmx.resolve())
        )
        self.assertEqual(
            report["trajectory_analysis"]["environment_component"], "gmx-mmpbsa"
        )
        self.assertTrue(report["trajectory_analysis"]["ready"])
        self.assertIsNone(report["trajectory_analysis"]["reason"])

    def test_audit_ready_false_when_gmx_on_path_but_mmpbsa_missing(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)

            def fake_which(name):
                return "/usr/bin/gmx" if name == "gmx" else None

            def fake_run(args, **kwargs):
                return subprocess.CompletedProcess(args, 0, "GROMACS 2023 test\n", "")

            with (
                mock.patch.object(ENVIRONMENT.shutil, "which", side_effect=fake_which),
                ...
                report = ENVIRONMENT.audit("wsl2-gpu", root)
        self.assertTrue(report["tools"]["gmx"]["available"])
        self.assertFalse(report["tools"]["gmx_MMPBSA"]["available"])
        self.assertFalse(report["trajectory_analysis"]["ready"])
        self.assertEqual(
            report["trajectory_analysis"]["reason"],
            "gmx-mmpbsa component is not installed",
        )
    ...
    def test_cpu_readiness_requires_gromacs(self):
        report = {
            "tools": {"uv": {"available": True}, "gmx": {"available": False}},
            "host": {"wsl": False},
            "diagnostics": {},
        }
    ...
    def test_host_recheck_writes_read_only_receipt(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                receipt["artifact_type"], "molecular_modeling_host_recheck_receipt"
            )
    ...
    def test_isolated_wsl_context_is_not_reported_as_host_failure(self):
        report = {
            ...
            "execution_context": {
                ...
                "wsl_gpu_device": {"exists": False},
                "docker_socket": {"exists": True, "gid": 65534},
            },
        }
...
class OnboardTests(unittest.TestCase):
    def test_onboard_writes_json_and_markdown_checklist(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                checklist["artifact_type"], "molecular_modeling_onboarding_checklist"
            )
            summary = checklist["summary"]
            self.assertEqual(summary["total"], len(checklist["steps"]))
            self.assertEqual(
                summary["total"],
                summary["done"] + summary["action_needed"] + summary["handoff"],
            )
    ...
    def test_onboard_covers_components_and_handoffs(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            missing = [
                step for step in checklist["steps"] if step["status"] == "action-needed"
            ]
            for step in missing:
                self.assertIn("command", step)
            chimerax = next(
                step for step in checklist["steps"] if "ChimeraX" in step["title"]
            )
