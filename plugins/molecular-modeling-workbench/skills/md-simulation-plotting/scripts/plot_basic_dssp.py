...
_DSSP_STATES = {
    "~": ("#F0F0F0", "Coil"), "H": ("#D73027", "Alpha helix"),
    "B": ("#4575B4", "Beta bridge"), "E": ("#74ADD1", "Beta strand"),
    "G": ("#F46D43", "3-10 helix"), "I": ("#A50026", "Pi helix"),
    "T": ("#FDAE61", "Turn"), "S": ("#ABDDA4", "Bend"),
}
...
def plot_tier1_dssp(sys1_xpm, sys2_xpm=None, output_name="tier1_dssp", sys1_label="System 1", sys2_label="System 2"):
    """
    ...
    fig, axes = plt.subplots(n_rows, 1, figsize=(6.8, 2.0 * n_rows), sharex=True)
    if not is_single:
    ...
    # System 1
    im1 = ax1.imshow(mat1, aspect='auto', cmap=cmap, norm=norm, origin='upper', extent=extent1, interpolation='none')
    ...
    if is_single:
        ...
        im2 = ax2.imshow(mat2, aspect='auto', cmap=cmap, norm=norm, origin='upper', extent=extent2, interpolation='none')
        ax2.set_ylabel("Residue Index")
        ax2.set_xlabel(x_label)
        ax2.set_title(f"B. {sys2_label} Secondary Structure", fontweight='bold', loc='left')
    
    from matplotlib.patches import Patch
    legend_elements = [Patch(facecolor=colors[i], label=labels[i]) for i in range(len(colors)) if labels[i] != "Coil"]
