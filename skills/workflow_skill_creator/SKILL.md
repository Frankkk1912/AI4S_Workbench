...
### Design Philosophy: Balancing Constraints and Openness

When designing a skill, you must strike a careful balance between engineering rigor and scientific flexibility:

-   **Constraints (The Guardrails)**: Hardcode strict engineering rituals where mistakes are catastrophic. Use absolute constraints for hardware settings, environment configs, known bug workarounds, or critical boilerplate parameters. Prevent the agent from guessing or hallucinating in areas where there is only one "right" way to do it.
-   **Openness (The Flexibility)**: Leave scientific decisions and analytical paths open. Do not hardcode specific file names, biological entity names, or downstream analysis choices unless explicitly requested. Provide patterns and examples, allowing the agent to use its reasoning to adapt the skill to new, unseen biological systems.
