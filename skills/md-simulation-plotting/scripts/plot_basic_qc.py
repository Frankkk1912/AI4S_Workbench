...
def plot_tier1_qc(sys1_rmsd, sys2_rmsd, sys1_rmsf, sys2_rmsf, 
                 sys1_rg, sys2_rg, sys1_sasa, sys2_sasa, 
                 output_name="tier1_stability_solvent", 
                 sys1_label="System 1", sys2_label="System 2", eq_start_ns=20,
                 rmsd_ylabel="Backbone RMSD (nm)", rmsf_ylabel="Cα RMSF (nm)", rmsf_xlabel="Residue Number"):
    """
    ...
    if not is_single:
        ...
        if t2_rmsd.max() > 1000: t2_rmsd /= 1000
        if t2_rg.max() > 1000: t2_rg /= 1000
        if t2_sasa.max() > 1000: t2_sasa /= 1000

    # Convert ps to ns for System 1 if needed
    if t1_rmsd.max() > 1000: t1_rmsd /= 1000
    if t1_rg.max() > 1000: t1_rg /= 1000
    if t1_sasa.max() > 1000: t1_sasa /= 1000
    ...
    ax.fill_between(t1_rmsd, mean1 - std1, mean1 + std1, color=md_style.StyleConfig.SYS1_COLOR, alpha=0.12, zorder=1)
    ax.plot(t1_sm, y1_sm, color=md_style.StyleConfig.SYS1_COLOR, lw=1.2, label=f"{sys1_label}: {mean1:.2f}±{std1:.2f} nm")
    ...
    if not is_single:
        ...
        ax.fill_between(t2_rmsd, mean2 - std2, mean2 + std2, color=md_style.StyleConfig.SYS2_COLOR, alpha=0.12, zorder=1)
        ax.plot(t2_sm, y2_sm, color=md_style.StyleConfig.SYS2_COLOR, lw=1.2, label=f"{sys2_label}: {mean2:.2f}±{std2:.2f} nm")
    ...
    # =========================================================================
    ax = axes[0, 1]
    ax.plot(res1, y1_rmsf, color=md_style.StyleConfig.SYS1_COLOR, alpha=0.85, lw=0.8, label=sys1_label)
    if not is_single:
        ax.plot(res2, y2_rmsf, color=md_style.StyleConfig.SYS2_COLOR, alpha=0.85, lw=0.8, label=sys2_label)
    ...
    ax.fill_between(t1_rg, mean1_rg - std1_rg, mean1_rg + std1_rg, color=md_style.StyleConfig.SYS1_COLOR, alpha=0.12)
    ax.plot(t1_rg[w-1:], y1_rg_sm, color=md_style.StyleConfig.SYS1_COLOR, lw=1.2, label=f"{sys1_label}: {mean1_rg:.2f} nm")
    ...
    if not is_single:
        ...
        ax.fill_between(t2_rg, mean2_rg - std2_rg, mean2_rg + std2_rg, color=md_style.StyleConfig.SYS2_COLOR, alpha=0.12)
        ax.plot(t2_rg[w-1:], y2_rg_sm, color=md_style.StyleConfig.SYS2_COLOR, lw=1.2, label=f"{sys2_label}: {mean2_rg:.2f} nm")
    ...
    ax.plot(t1_sasa[w-1:], y1_sasa_sm, color=md_style.StyleConfig.SYS1_COLOR, lw=1.2, label=sys1_label)
    
    if not is_single:
        y2_sasa_sm = parser_gmx.convolve_smooth(y2_sasa, w)
        p_val = parser_gmx.block_average_welch(y1_sasa[t1_sasa >= eq_start_ns], y2_sasa[t2_sasa >= eq_start_ns])
        stars = parser_gmx.get_p_value_stars(p_val)
        ax.plot(t2_sasa[w-1:], y2_sasa_sm, color=md_style.StyleConfig.SYS2_COLOR, lw=1.2, label=f"{sys2_label} (p={p_val:.2e} {stars})")
