...
def plot_tier3_energy(sys1_energy, sys2_energy, sys1_hb, sys2_hb, 
    ...
    if t1_hb.max() > 1000: t1_hb /= 1000
    if not is_single:
        t2_hb, y2_hb = parser_gmx.read_xvg(sys2_hb, [0, 1])
        if t2_hb.max() > 1000: t2_hb /= 1000
    ...
    if has_energy:
        ...
        fig, axes = plt.subplots(1, 2, figsize=(6.8, 3.0), gridspec_kw={'width_ratios': [1.5, 1]})
        ...
        if is_energy_single:
            width = 0.6
            ax.bar(x, dg1, width, yerr=sem1, capsize=2,
                   label=sys1_label, color=md_style.StyleConfig.SYS1_COLOR, edgecolor='white', linewidth=0.5)
        else:
            width = 0.35
            ax.bar(x - width/2, dg1, width, yerr=sem1, capsize=2,
                   label=sys1_label, color=md_style.StyleConfig.SYS1_COLOR, edgecolor='white', linewidth=0.5)
            x2 = np.arange(len(res2))
            ax.bar(x2 + width/2, dg2, width, yerr=sem2, capsize=2,
                   label=sys2_label, color=md_style.StyleConfig.SYS2_COLOR, edgecolor='white', linewidth=0.5)
        ...
        ax.set_title("A. Per-Residue Binding Energy Breakdown", fontweight='bold', loc='left')
        ax.legend(loc='upper right', frameon=False)
        
        ax_hb = axes[1]
    else:
        # Fallback to 1x1 H-bond plot
        fig, ax_hb = plt.subplots(figsize=(3.4, 3.0))
    ...
    ax.plot(t1_hb[w-1:], y1_hb_sm, color=md_style.StyleConfig.SYS1_COLOR, lw=1.2, 
    ...
    if not is_single:
        ...
        ax.plot(t2_hb[w-1:], y2_hb_sm, color=md_style.StyleConfig.SYS2_COLOR, lw=1.2, 
                label=f"{sys2_label} (mean={mean2_hb:.1f})")
    
    ax.set_xlabel("Time (ns)")
    ax.set_ylabel("Number of H-bonds")
    ax.set_title("B. Interfacial Hydrogen Bonds" if has_energy else "Interfacial Hydrogen Bonds", fontweight='bold', loc='left')
