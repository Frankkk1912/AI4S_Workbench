import { z } from 'zod';
import { defaultDataDir, defaultFulltextExchangeRoot } from './lib/paths.js';
...
export interface ZoteusConfig {
  ...
  dataDir: string;
  fulltextExchangeRoot: string;
  contactEmail?: string;
  allowDelete: boolean;
  readOnly: boolean;
  toolProfile: 'full' | 'workbench';
...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): ZoteusConfig {
  const schema = z.object({
    ...
    LITERATURE_ZOTERO_MCP_TOOL_PROFILE: z.enum(['full', 'workbench']).default('full'),
  ...
  return {
    ...
    fulltextExchangeRoot: parsed.LITERATURE_FULLTEXT_EXCHANGE_ROOT ?? defaultFulltextExchangeRoot(env),
    contactEmail: parsed.ZOTEUS_CONTACT_EMAIL,
    allowDelete: parsed.ZOTEUS_ALLOW_DELETE,
    readOnly: parsed.ZOTEUS_READ_ONLY,
    toolProfile: parsed.LITERATURE_ZOTERO_MCP_TOOL_PROFILE,
