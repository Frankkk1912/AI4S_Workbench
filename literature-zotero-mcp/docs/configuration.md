...
For first-time setup, run `npm run zotero:configure -- setup` from the built
runtime. It uses a masked local prompt, validates the key, and writes it to the
platform's private configuration directory. At startup the server also loads:

- macOS: `~/Library/Application Support/ai4s-literature-workbench/zotero.env`
- Windows: `%LOCALAPPDATA%\\ai4s-literature-workbench\\zotero.env`
- Linux/WSL: `${XDG_CONFIG_HOME:-~/.config}/ai4s-literature-workbench/zotero.env`

Set `LITERATURE_ZOTERO_MCP_ENV_FILE` to point at a different private env file.
Values already present in the process environment keep precedence.
The previous `~/.config/ai4s-literature-workbench/zotero.env` location remains
